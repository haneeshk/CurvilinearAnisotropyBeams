...
function EI = Jolicoeur(radiiVec,I,alpha,flag_mat,flag_sym,flag_intf)

N=length(radiiVec)-1;

% Set up the material property materices
% CMat=ComposeCMat(N);
CMat=ReadCMat(N,alpha,flag_mat,flag_sym);
...
fprintf(fidA,'%d %s\n',I,'Layers');
fprintf(fidB,'%s %5.2f %d %s \n','alpha=',alpha*180/pi,I,'Layers');
fprintf(fidX,'%s %5.2f %d %s \n','alpha=',alpha*180/pi,I,'Layers');
%fprintf(fide,'%d %s\n',I,'Layers');
for i=1:7*I
    for j=1:7*I
...
fprintf(fidr,'%s %5.2f','alpha=',alpha*180/pi);
